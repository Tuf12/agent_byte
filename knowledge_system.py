# knowledge_system.py - How Agent Byte Uses Symbolic Knowledge
import numpy as np
import random
...
class KnowledgeInterpreter:
    ...
    def apply_knowledge(self, state: np.ndarray, q_values: np.ndarray, app_context: Dict, 
                       exploration_rate: float) -> Tuple[int, str]:
        ...
        return np.argmax(q_values), f"Strategy '{chosen_strategy}' not implemented"
    ...
    def _choose_best_strategy(self, strategies: List[str], game_situation: Dict) -> str:
        ...
        if urgency > 0.7:
            defensive_strategies = [s for s in strategies if any(word in s.lower() 
                                  for word in ['defensive', 'timing', 'intercept', 'reaction'])]
        ...
        # Medium urgency: prioritize predictive/positioning strategies  
            ...
            predictive_strategies = [s for s in strategies if any(word in s.lower()
                                   for word in ['predictive', 'trajectory', 'positioning', 'anticipate'])]
        ...
        # Low urgency: prioritize offensive/angle strategies
            ...
            offensive_strategies = [s for s in strategies if any(word in s.lower()
                                  for word in ['angle', 'aggressive', 'offensive', 'counter'])]
    ...
    def _apply_aggressive_returns(self, state: np.ndarray, q_values: np.ndarray,
        ...
        if game_situation['ball_approaching'] and game_situation['urgency'] > 0.6:
    ...
    def _apply_failure_avoidance(self, action: int, state: np.ndarray, 
        ...
        # Avoid reactive play
        if 'reactive_play' in failure_patterns and game_situation['urgency'] < 0.3:
    ...
    def _avoid_reactive_play(self, action: int, state: np.ndarray, game_situation: Dict) -> int:
        ...
        if game_situation['urgency'] < 0.3:
            ball_y = game_situation['ball_position'][1]
            paddle_y = game_situation['paddle_position']
    ...
    def _avoid_passive_defense(self, action: int, state: np.ndarray, game_situation: Dict) -> int:
        ...
        if game_situation['urgency'] < 0.4 and not game_situation['ball_approaching']:
            # Not defensive situation - can be more aggressive
            return action  # Allow more aggressive positioning
        
        return action
    
    # Helper methods for other strategies (simplified implementations)
    def _apply_angle_control(self, state, q_values, game_situation):
        return self._apply_aggressive_returns(state, q_values, game_situation)
    
    def _apply_counter_attacking(self, state, q_values, game_situation):
        return self._apply_aggressive_returns(state, q_values, game_situation)
    
    def _apply_ball_tracking(self, state, q_values, game_situation):
        return self._apply_trajectory_prediction(state, q_values, game_situation)
    
    def _apply_angle_shots(self, state, q_values, game_situation):
        return self._apply_aggressive_returns(state, q_values, game_situation)
    
    def _apply_early_interception(self, state, q_values, game_situation):
        return self._apply_early_positioning(state, q_values, game_situation)
    
    def _apply_consistency_focus(self, state, q_values, game_situation):
        return self._apply_optimal_positioning(state, q_values, game_situation)
    
    def _apply_offensive_angles(self, state, q_values, game_situation):
        return self._apply_aggressive_returns(state, q_values, game_situation)
    
    def _apply_defense_to_offense(self, state, q_values, game_situation):
        if game_situation['urgency'] > 0.7:
...
class SymbolicDecisionMaker:
    ...
    def make_informed_decision(self, state: np.ndarray, q_values: np.ndarray, 
        ...
        if use_symbolic:
            ...
            return symbolic_action, f"🧩 {reasoning}"
            ...
            return nn_action, "🧠 Neural network decision"
